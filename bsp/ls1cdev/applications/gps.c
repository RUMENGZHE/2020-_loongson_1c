...
void NMEA_BDGSV_Analysis(nmea_msg *gpsx,rt_uint8_t *buf)
{
	...
	rt_uint8_t posx;   	 
	p=buf;
	p1=(rt_uint8_t*)strstr((const char *)p,"$BDGSV");
...
void NMEA_GNGSA_Analysis(nmea_msg *gpsx,rt_uint8_t *buf)
{
	...
	rt_uint8_t posx; 
	rt_uint8_t i;   
	p1=(rt_uint8_t*)strstr((const char *)buf,"$GNGSA");
...
void NMEA_GNRMC_Analysis(nmea_msg *gpsx,rt_uint8_t *buf)
{
	...
	rt_uint32_t temp;	   
	float rs;  
	p1=(rt_uint8_t*)strstr((const char *)buf,"$GNRMC");//"$GNRMC",经常有&和GNRMC分开的情况,故只判断GPRMC.
